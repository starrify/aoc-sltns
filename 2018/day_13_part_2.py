...
def main():
    ...
    while carts.qsize() > 1:
        ...
        while carts.qsize():
            ...
            if pos in removed_locs:
                removed_locs.remove(pos)
                continue
            new_pos = tuple(
            ...
            if new_pos in cart_locs:
                cart_locs.remove(new_pos)
                cart_locs.remove(pos)
                removed_locs.add(new_pos)
                continue
            else:
        ...
        carts = queue.PriorityQueue()
        while tmp_carts.qsize():
            cart = tmp_carts.get()
            if cart[0] in removed_locs:
                removed_locs.remove(cart[0])
                continue
            carts.put(cart)
        if False:
            tmp_carts = queue.PriorityQueue()
