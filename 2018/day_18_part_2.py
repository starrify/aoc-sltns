...
def main():
    ...
    while iteration > 0:
        ...
        canvas = new_canvas
        iteration -= 1
        signature = ''.join(x for row in canvas for x in row)
        if signature in signature_iteration:
            cycle = signature_iteration[signature] - iteration
            iteration %= cycle
        signature_iteration[signature] = iteration
    tmp = [
